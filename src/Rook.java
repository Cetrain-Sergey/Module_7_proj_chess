public class Rook extends ChessPiece {
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int startLine, int startColumn,
        ...
        if (checkpos(startLine) && checkpos(startColumn) && checkpos(endLine) && checkpos(endColumn)) {
            ...
            if (isUnderAttack(chessBoard, endLine, endColumn))
                return false;
    ...
    private boolean isUnderAttack(ChessBoard chessBoard, int endLine, int endColumn) {
        if (checkpos(endLine) && checkpos(endColumn)) {
            for (int i = 0; i < 7; i++) {
                for (int j = 0; j < 7; j++) {
                    if (chessBoard.board[i][j] != null) {
                        if (!chessBoard.board[i][j].getColor().equals(color) &&
                                chessBoard.board[i][j].canMoveToPosition(chessBoard, i, j,
                                        endLine, endColumn)) {
                            return true;
                        }
                    }
